  ...
  const renderConversation = useCallback(
    ({ item }) =>
      normalizeVietnamese(item?.name as string).includes(
        normalizeVietnamese(filterMessageInput.trim().toLowerCase())
      ) ? (
        <TouchableOpacity
    ...
    [handleConversationSelect, filterMessageInput]
  );
          ...
          <View className="p-4">
            <View className="flex-row items-center bg-gray-100 rounded-full px-4 py-2">
              ...
              <TextInput
                ...
                value={filterMessageInput}
                onChangeText={(e) => setFilterMessageInput(e)}
              />
              {filterMessageInput.trim().length > 0 && (
